...
function referentiel_print_etablissement($record){
	...
	if ($record){
		$s.= '<tr bgcolor="white"><td>'.stripslashes($record->num_etablissement);
	    $s.= '</td><td>'.stripslashes($record->nom_etablissement);
		$s.= '</td><td>'.stripslashes($record->adresse_etablissement);
...
//         $s.='&nbsp; <a href="'.$CFG->wwwroot.'/mod/referentiel/etablissement.php?d='.$referentiel_id.'&etablissement_id='.$etablissement_id.'&mode=updateetablissement&sesskey='.sesskey().'"><img src="'.$CFG->pixpath.'/t/edit.gif" alt="'.get_string('edit').'" title="'.get_string('edit').'" /></a>'."\n";
        $s.='&nbsp; <a href="'.$CFG->wwwroot.'/mod/referentiel/etablissement.php?d='.$referentiel_id.'&etablissement_id='.$etablissement_id.'&mode=updateetab&sesskey='.sesskey().'"><img src="'.$OUTPUT->pix_url('/t/edit').'" alt="'.get_string('edit').'" title="'.get_string('edit').'" /></a>'."\n";
	    $s.='&nbsp; <a href="'.$CFG->wwwroot.'/mod/referentiel/etablissement.php?d='.$referentiel_id.'&etablissement_id='.$etablissement_id.'&mode=deleteetab&sesskey='.sesskey().'"><img src="'.$OUTPUT->pix_url('/t/delete').'" alt="'.get_string('delete').'" title="'.get_string('delete').'" /></a>'."\n";
	}
	return $s;
}
