...
// AFFICHAGE DE LA PAGE Moodle 2
	...
	$strmessage = get_string('verroucertificat','referentiel');
    ...
    echo '<div align="center"><h2><img src="'.$icon.'" border="0" title="" alt=""> '.$strmessage.' '.$OUTPUT->help_icon('verroucertificath','referentiel').'</h2></div>'."\n";

    if (!empty($action) && ($action=='verrou')) {  // La selection est faite
        //
...
?>
            ...
            <table cellpadding="5">
                <tr>
                    <td>
        <?php
        echo '<b>'.get_string('verrou_all', 'referentiel').'</b><input type="radio" name="verrou" value="1"  checked="checked" /> '."\n";
        ?>
        </td><td> &nbsp; &nbsp; &nbsp;</td><td>
        <?php
        echo '<b>'.get_string('deverrou_all', 'referentiel').'</b><input type="radio" name="verrou" value="0"  /> ' ."\n";
        ?>
                        </td>
                </tr>
                <tr>
        ...
        referentiel_select_liste_certificats($referentiel, $list_pedagoids, $userid_filtre, $gusers, $select_acc, $mode, $CFG->wwwroot . '/mod/referentiel/verrou_certificat.php?d='.$referentiel->id, $select_all, $sql_filtre_where, $data_filtre);
    }
    echo $OUTPUT->footer();
    die();
