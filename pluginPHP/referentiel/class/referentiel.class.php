...
class referentiel {
    ...
    */
     ...
     */
    function view($mode, $currenttab, $select_acc) {
       ...
       global $CFG, $USER;
        ...
        if (!$this->get_referentiel_referentiel()){
            ...
            $this->add_referentiel_referentiel();
                ...
                $this->view_intro();
                $this->onglets($mode, $currenttab, $select_acc);
                $this->view_title();
...
<input type="hidden" name="description_instance" value="'.$this->referentiel->description_instance  .'" />
...
<input type="hidden" name="mode" value="'.$mode.'" />
    ...
    function view_instance_referentiel() {
        ...
        echo '    <td>'.htmlentities($this->referentiel->name, ENT_QUOTES) .'</td></tr>';
        echo '<tr><th>'.get_string('description_instance','referentiel').':</th>';
        echo '    <td>'.htmlentities($this->referentiel->description_instance, ENT_QUOTES).'</td></tr>';
     ...
     *
     */
    function onglets($mode, $currenttab, $select_acc) {
    ///////////// TABS ////////////////
    global $USER;
    global $CFG;
        ...
        $isadmin=referentiel_is_admin($USER->id, $this->course->id);
        ...
        if (has_capability('mod/referentiel:view', $this->context)) {
        	$row[] = new tabobject('referentiel', $CFG->wwwroot.'/mod/referentiel/view.php?d='.$this->referentiel->id.'&select_acc='.$select_acc.'&non_redirection=1', get_string('referentiel','referentiel'));
        }
        ...
        if (isloggedin()) {
            ...
            if (referentiel_user_can_addactivity($this->referentiel)) {
                ...
                $row[] = new tabobject('list', $CFG->wwwroot.'/mod/referentiel/activite.php?d='.$this->referentiel->id.'&select_acc='.$select_acc.'&mode=list', $addstring);
            }
            ...
            if (has_capability('mod/referentiel:addtask', $this->context) || has_capability('mod/referentiel:viewtask', $this->context)) {
                ...
                $row[] = new tabobject('task', $CFG->wwwroot.'/mod/referentiel/task.php?d='.$this->referentiel->id.'&select_acc='.$select_acc.'&mode=listtask', $addstring);
            }


            // gestion des certificats
            if (has_capability('mod/referentiel:write', $this->context)) {
                $row[] = new tabobject('certificat', $CFG->wwwroot.'/mod/referentiel/certificat.php?d='.$this->referentiel->id.'&select_acc='.$select_acc.'&mode=listcertif',
            ...
            if (isset($currenttab) && (($currenttab == 'list')
                ...
                $row  = array();
                $inactive[] = 'list';
		        $row[] = new tabobject('listactivity', $CFG->wwwroot.'/mod/referentiel/activite.php?d='.$this->referentiel->id.'&select_acc='.$select_acc.'&mode=listactivity',  get_string('listactivity','referentiel'));
	           	$row[] = new tabobject('listactivityall', $CFG->wwwroot.'/mod/referentiel/activite.php?d='.$this->referentiel->id.'&select_acc='.$select_acc.'&mode=listactivityall',  get_string('listactivityall','referentiel'));

                // accompagnement
                if (has_capability('mod/referentiel:write', $this->context)) {
                    $row[] = new tabobject('accompagnement', $CFG->wwwroot.'/mod/referentiel/activite.php?d='.$this->referentiel->id.'&select_acc='.$select_acc.'&mode=accompagnement', get_string('accompagnement','referentiel'));
                }
                if (has_capability('mod/referentiel:addactivity', $this->context)) {
                    $row[] = new tabobject('addactivity', $CFG->wwwroot.'/mod/referentiel/activite.php?d='.$this->referentiel->id.'&select_acc='.$select_acc.'&mode=addactivity',  get_string('addactivity','referentiel'));
                }
                if (!has_capability('mod/referentiel:managecertif', $this->context)) {      // rôle etudiant : uniquement pour modifier une activite
                    if ($mode=='updateactivity'){
				        $row[] = new tabobject('updateactivity', $CFG->wwwroot.'/mod/referentiel/activite.php?d='.$this->referentiel->id.'&select_acc='.$select_acc.'&mode=updateactivity',  get_string('updateactivity','referentiel'));
    			}
            }
            else {
                $row[] = new tabobject('updateactivity', $CFG->wwwroot.'/mod/referentiel/activite.php?d='.$this->referentiel->id.'&select_acc='.$select_acc.'&mode=updateactivity',  get_string('updateactivity','referentiel'));
            ...
            if (has_capability('mod/referentiel:export', $this->context)) {
    			$row[] = new tabobject('exportactivity', $CFG->wwwroot.'/mod/referentiel/export_activite.php?d='.$this->referentiel->id.'&select_acc='.$select_acc.'&mode=exportactivity',  get_string('export','referentiel'));
            }
	        $tabs[] = $row;
		...
		|| ($currenttab == 'importtask')
			...
			$row[] = new tabobject('listtask', $CFG->wwwroot.'/mod/referentiel/task.php?d='.$this->referentiel->id.'&select_acc='.$select_acc.'&mode=listtask',  get_string('listtask','referentiel'));
			$row[] = new tabobject('listtasksingle', $CFG->wwwroot.'/mod/referentiel/task.php?d='.$this->referentiel->id.'&select_acc='.$select_acc.'&mode=listtasksingle',  get_string('listtasksingle','referentiel'));
		}

	    if (has_capability('mod/referentiel:addtask', $this->context)) {
			$row[] = new tabobject('addtask', $CFG->wwwroot.'/mod/referentiel/task.php?d='.$this->referentiel->id.'&select_acc='.$select_acc.'&mode=addtask',  get_string('addtask','referentiel'));
			$row[] = new tabobject('updatetask', $CFG->wwwroot.'/mod/referentiel/task.php?d='.$this->referentiel->id.'&select_acc='.$select_acc.'&mode=updatetask',  get_string('updatetask','referentiel'));
		}
	...
	// CERTIFICATS
		...
		if (has_capability('mod/referentiel:view', $this->context)) { // afficher un certificat
      	    $row[] = new tabobject('listcertif', $CFG->wwwroot.'/mod/referentiel/certificat.php?d='.$this->referentiel->id.'&select_acc='.$select_acc.'&mode=listcertif&sesskey='.sesskey(), get_string('listcertif', 'referentiel'));
      	    $row[] = new tabobject('editcertif', $CFG->wwwroot.'/mod/referentiel/certificat.php?d='.$this->referentiel->id.'&select_acc='.$select_acc.'&mode=editcertif&sesskey='.sesskey(), get_string('editcertif', 'referentiel'));
            if (referentiel_site_can_print_graph($this->referentiel->id) ){
                $row[] = new tabobject('statcertif', $CFG->wwwroot.'/mod/referentiel/certificat.php?d='.$this->referentiel->id.'&select_acc='.$select_acc.'&mode=statcertif&sesskey='.sesskey(), get_string('statcertif', 'referentiel'));
            }
		}
	...
	// REFERENTIELS
        ...
        if ((isset($isadmin) && $isadmin) || (isset($isreferentielauteur) && $isreferentielauteur)
            || referentiel_site_can_write_or_import_referentiel($this->referentiel->id)) {
...
<input type="hidden" name="name_instance" value="'. htmlentities($this->referentiel->name, ENT_QUOTES).'" />
<input type="hidden" name="description_instance" value="'.htmlentities($this->referentiel->description_instance, ENT_QUOTES).'" />
<input type="hidden" name="label_domaine" value="'.$this->referentiel->label_domaine.'" />
<input type="hidden" name="label_competence" value="'.$this->referentiel->label_competence.'" />
<input type="hidden" name="label_item" value="'.$this->referentiel->label_item.'" />
...
<input type="hidden" name="label_competence" value="'.$this->referentiel->label_competence.'" />
<input type="hidden" name="label_item" value="'.$this->referentiel->label_item.'" />
<input type="hidden" name="intro" value="'.htmlentities($this->referentiel->intro, ENT_QUOTES).'" />
...
<table class="saisie">
<tr valign="top">
    <td class="saisie" align="right"><b>'.htmlentities(get_string('name','referentiel'), ENT_QUOTES)   .':</b></td>
    <td class="saisie" align="left">
        <input type="text" name="name" size="60" maxlength="80" value="" />
    </td>
</tr>
<tr valign="top">
    <td class="saisie" align="right"><b>'.htmlentities(get_string('code','referentiel'), ENT_QUOTES)  .':</b></td>
    <td class="saisie" align="left">
        <input type="text" name="code_referentiel" size="20" maxlength="20" value="" />
    </td>
</tr>
<tr valign="top">
    <td class="saisie" align="right"><b>'.get_string('pass_referentiel','referentiel').'</b> : <br /><span="small"><i>'.get_string('aide_pass_referentiel','referentiel').'</i></span>' .'</td>
    <td class="saisie" align="left">
...
<tr valign="top">
    <td class="saisie" align="right"><b>'.htmlentities(get_string('description','referentiel'), ENT_QUOTES)  .':</b></td>
    <td class="saisie" align="left">
...
<tr valign="top">
    <td class="saisie_domaine" align="left" colspan="2"><i>'.get_string('domaine','referentiel')  .'</i></td>
</tr>
...
<tr valign="top">
    <td class="saisie_domaine" align="right"><b>'.htmlentities(get_string('description','referentiel'), ENT_QUOTES)  .':</b></td>
    <td class="saisie_domaine" align="left">
...
<tr valign="top">
    <td class="saisie_competence" align="left" colspan="2"><i>'.get_string('competence','referentiel')  .'</i></td>
</tr>
<tr valign="top">
...
<tr valign="top">
    <td class="saisie_competence" align="right"><b>'.htmlentities(get_string('description','referentiel'), ENT_QUOTES)  .':</b></td>
    <td class="saisie_competence" align="left">
...
<tr valign="top">
    <td class="saisie_item" align="right"><b>'.htmlentities(get_string('description','referentiel'), ENT_QUOTES)  .':</b></td>
    <td class="saisie_item" align="left">
...
<tr valign="top">
    ...
    <td class="saisie_item" align="left">
        <input type="text" name="type_item" size="20" maxlength="20" value="" />
    </td>
</tr>
<tr valign="top">
