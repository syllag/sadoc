    ...
    // $file_uri= 2/moddata/referentiel/1/3/arrete-C2i2eVDef.pdf
        ...
        if (referentiel_check_and_create_document_files_dir($referentiel_referentiel_id, $user_creator, $userid)){
            ...
            if (!empty($file_name)){
                ...
                //exit;
                $status = referentiel_copy_file($file_url,
                    $CFG->dataroot."/temp/archive/".$referentiel_referentiel_id."/".$user_creator."/".$file_path_name);
            }
        }
        if ($status){
 ...
 */
    ...
    // Moodle 2 file API
        ...
        if (is_file($from_file)) {
            //Debug
            // echo "<br />DEBUG :: lib_archive.php :: 264 :: Copying ".$from_file." to ".$filename;
 ...
 * @param int user_filtre : user owner of file to display
            ...
            if ($userid && (($userid_filtre==0) || ($userid==$userid_filtre))){
                ...
                if ($ok_portfolio) {
                    ...
                    $button->set_callback_options('referentiel_portfolio_caller',
                        array('instanceid' => $instanceid, 'attachment' => $file->get_id(), 'report' => $report, 'export_format' => $export_format), '/mod/referentiel/locallib_portfolio.php');
