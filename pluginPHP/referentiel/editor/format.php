...
class rformat_xml extends rformat_default {
    ...
    $nbitems=0;              // compteur
        ...
        $re->code_referentiel = $this->getpath( $xmlreferentiel, array('#','idcode',0,'#'), '', true, $error_nocode);
        $re->description_referentiel = $this->getpath( $xmlreferentiel, array('#','definition',0,'#','text',0,'#'), '', true, '');
        $re->url_referentiel = $this->getpath( $xmlreferentiel, array('#','url',0,'#'), '', true, '');
		// $re->seuil_certificat = $this->getpath( $xmlreferentiel, array('#','seuil_certificat',0,'#'), '', false, '');
		$re->seuil_certificat = 0;
...
*/
		...
		if (!empty($xmlreferentiel['#']['domaine'])){
            ...
            foreach ($xmldomaines as $domaine) {
                ...
                $new_domaine->code_domaine=$this->getpath( $domaine, array('#','idcode',0,'#'), '', true, $error_nocode);
                $new_domaine->description_domaine=$this->getpath( $domaine, array('#','definition',0,'#','text',0,'#'), '', true, '');
                //$new_domaine->num_domaine=$this->getpath( $domaine, array('#','num_domaine',0,'#'), '', false, '');
                $new_domaine->num_domaine=$dindex;
	            ...
	            foreach ($xmlcompetences as $competence) {
				    ...
				    $new_competence->code_competence=$this->getpath( $competence, array('#','idcode',0,'#'), '', true, $error_nocode);
				    $new_competence->description_competence=$this->getpath( $competence, array('#','definition',0,'#','text',0,'#'), '', true, '');
				    // $new_competence->num_competence=$this->getpath( $competence, array('#','num_competence',0,'#'), '', false, '');
                    $new_competence->num_competence=$cindex;
		            ...
		            foreach ($xmlitems as $item) {
					    ...
					    $new_item->code_item = $this->getpath( $item, array('#','idcode',0,'#'), '', true, $error_nocode);
					    $new_item->description_item=$this->getpath( $item, array('#','definition',0,'#','text',0,'#'), '', true, '');
					    //$new_item->num_item=$this->getpath( $item, array('#','num_item',0,'#'), '', false, '');
					    $new_item->num_item=$iindex               ;
