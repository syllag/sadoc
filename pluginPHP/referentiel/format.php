    ...
    // MOODLE 2.0
        if (!empty($this->contents)) {
            ...
            if (ereg("\r", $this->contents) AND !ereg("\n", $this->contents)) {
                return explode("\r", $this->contents);
            } else {
                return  explode("\r\n", $this->contents);
            }
        }
 ...
 ***********************/
    ...
    function try_importing( $data, $pedagos=null, $extra=null ) {
        ...
        // function to handle this referentiel
        if (method_exists( $methodname)) {
        	if ($pedagos = $methodname( $data, $referentiel, $this, $extra )) {
            	return $pedagos;
            }
        }
        return false;   
...
class zformat_default {
    ...
    var $records_users=NULL; // liste des utilisateurs à archiver
 ...
 *******************/
    ...
    function exportprocess() {
        ...
        */
        ...
        $temp_path = $CFG->dataroot.'/'.$this->get_temp_dir();
        ...
        $filepath = $temp_path."/".$this->filename . $this->export_file_extension();
        if (!$fh=fopen($filepath,"w")) {
            $this->error( get_string('cannotopen','referentiel',$continuepath),$filepath,'','export' );
        }
		// DEBUG
    ...
    function exportpostprocess($archive_name) {
        ...
        $archive_name.='.zip';
        // Moodle 2 :
        // $fullpath = $CFG->dataroot.'/'.$this->context->id.'/mod_referentiel/archive/'.$this->get_export_dir().'/'.$archive_name;
        // echo "<br>DEBUG :: format.php/exportposprocess :: 3289 :  :: FILENAME : $archive_name<br>\n";
