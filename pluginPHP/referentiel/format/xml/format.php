...
class rformat_xml extends rformat_default {
    ...
    $nbitems=0;              // compteur
		...
		if (!isset($this->action) || (isset($this->action)
            && ($this->action!="selectreferentiel")
            && ($this->action!="importreferentiel"))){
		...
		*/
		...
		if (($risque_ecrasement==false) || ($this->newinstance==1)) {
			...
			// DEBUG
			// echo "<br />DEBUG xml/format.php ligne 588<br />NEW REFERENTIEL ID ENREGISTRE : ".$this->new_referentiel_id."\n";
		}
...
*/
    ...
    function write_certification() {
		...
		if ($this->ireferentiel){
			...
			// CERTIFICATS
			if (isset($this->ireferentiel->ref_referentiel) && ($this->ireferentiel->ref_referentiel>0)){
				...
				$records_certificats = referentiel_get_certificats($this->ireferentiel->ref_referentiel);
...
class tformat_xml extends tformat_default {
    ...
    global $CFG;
		...
		if ($consigne){
            ...
            if (!eregi("http", $url_consigne)){  // completer l'adresse relative
                $url_consigne=$CFG->wwwroot.'/file.php/'.$url_consigne;
            }
	...
	global $DB;
		...
		*/
        ...
        if (($referentiel_reconnu==false) && !empty($re->code_referentiel)
            && (strtoupper($this->rreferentiel->code_referentiel)==strtoupper($re->code_referentiel))){
				...
				if ($new_task_id){
					...
					if (!empty($xmltask['#']['consigne'])){
                        ...
                        foreach ($xmlconsignes as $xmlconsigne) {
                            ...
                            if (!eregi("http", $new_consigne->url_consigne)){
							   // Moodlel 1.9
							   // $url_consigne=$CFG->wwwroot.'/file.php/'.$url_consigne; // pas de liens relatifs sur les fichiers telecharges
                               // A REECRIRE POUR Moodle 2
	   ...
	   // selon les parametres soit cree une nouvelle instance
        ...
        foreach ($xmlpedagos as $pedago) {
			...
			if (!empty($formation) && !empty($pedagogie)  && !empty($composante)){
					    ...
					    $import_pedago->promotion =($promotion);
						$import_pedago->formation =($formation);
						$import_pedago->pedagogie =($pedagogie);
						$import_pedago->composante =($composante);
                        $import_pedago->num_groupe = ($num_groupe);
                        $import_pedago->commentaire =($commentaire);
