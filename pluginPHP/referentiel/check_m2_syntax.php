<?php // $Id: check_m2_syntax.php based on check_db_syntax.php,v 1.15 2011/02/12 11:49:15 stronk7 Exp $
...
// JF juin 2011, d'après check_db_syntax.php
...
/// List of patterns to search
    ...
    $newapi= array('number_format', 'isguest',
        'helpbutton', 'button_to_popup_window',
        'notice_yesno', 'choose_from_menu',
        'print(_user_picture|_table|_box_start|_box_end|_heading_with_help|_header_simple|_header|_footer)',
        'upload_print_form_fragment', 'build_navigation');

    $errorfunc= array('( )?error'  );

    $obsoleteconstants = array('PARAM_CLEAN');
    
    $pixlinks = array('\$CFG->pixpath');
    
    $fileapi= array('make_upload_directory');
...
/// All rules
    $all_megarules = array(
        'NEW_API'=>$newapi_megarule,
        'OBSOLETE_CONSTANTS'=>$obsoleteconstants_megarule,
        'PIX_LINKS'=>$pixlinks_megarule,
        'ERROR_FUNC'=>$errorfunc_megarule,
        'NEW_FILEAPI'=>$fileapi_megarule,
        'HTMLAPI'=>$htmlapi_megarule
    );
...
if (!$html){
                            ...
                            echo "          - ERROR ( $name ) - line $line : " . htmlentities(trim($buffer)) . LINEFEED;
}
                            $counterrors++;
                            break;
                        }
...
if (!$html){
            ...
            echo "        ".htmlentities($err) . LINEFEED;
}
        }
    }
