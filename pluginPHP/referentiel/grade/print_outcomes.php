<?php
    // affiche les notes et objectifs en rapport avec les referentiels de compétence
...
// affichage dans un onglet des modules d'activites utilisant les objectifs deu référentiel
...
// liste les activites du serveur evaluees par objectifs du référentiel
global $DB;
global $scales;
  //
    $objectifs = array();
    ...
    if ($r_referentiels){
        foreach($r_referentiels as $r_referentiel){
            ...
            */
            ...
            // Par ailleurs le courseid obtenu ne sert a rien car il peut être NULL
            // si les outcomes sont standards (définis au niveau du site
            // et non du cours
...
function referentiel_affiche_module_activite($notation){
    ...
    if (!empty($notation) && !empty($notation->module) && !empty($notation->moduleinstance) && !empty($notation->courseid)){
        ...
        .'</b> <b>'.get_string('module', 'referentiel').' '.get_string('modulename', $notation->module);
